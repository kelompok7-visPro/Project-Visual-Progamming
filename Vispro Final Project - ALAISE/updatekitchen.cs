...
{
    public partial class updatekitchen : Form
    {
        ...
        private void lblcari_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                query = "SELECT * FROM tbl_item_kitchen WHERE id_item_kitchen = @id_item_kitchen";
                perintah = new MySqlCommand(query, koneksi);
                perintah.Parameters.AddWithValue("@id_item_kitchen", id);
                ...
                if (reader.Read())
                {
                    ...
                    txtid.Text = reader["id_item_kitchen"].ToString();
        ...
        private void lblupdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    query = "UPDATE tbl_item_kitchen SET nama_item = @nama_item, kategori = @kategori, stok = @stok WHERE id_item_kitchen = @id_item_kitchen";
                    perintah = new MySqlCommand(query, koneksi);
        ...
        private void lbldelet_Click(object sender, EventArgs e)
        {
            try
            {
                // Mendapatkan nilai id dari txtid (textbox yang berisi id_item_kitchen untuk dihapus)
                string id = txtid.Text;
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    query = "DELETE FROM tbl_item_kitchen WHERE id_item_kitchen = @id_item_kitchen";
                    perintah = new MySqlCommand(query, koneksi);
                    perintah.Parameters.AddWithValue("@id_item_kitchen", id);
        ...
        private void updatekitchen_Load(object sender, EventArgs e)
        {
            try
            {
                koneksi.Open();
                query = string.Format("select * from tbl_item_kitchen");
                perintah = new MySqlCommand(query, koneksi);
