...
{
    public partial class kitchenadd : Form
    {
        ...
        private void kitchenadd_Load(object sender, EventArgs e)
        {
            try
            {
                koneksi.Open();
        ...
        private void lbladditem_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtid.Text != "" && txtnama.Text != "" && cbkategori.Text != "")
                {
                    ...
                    query = string.Format("insert into tbl_item_kitchen  values ('{0}','{1}','{2}','{3}');", txtid.Text, txtnama.Text, cbkategori.Text, txtstok.Text);
