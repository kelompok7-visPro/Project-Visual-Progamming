...
{
    public partial class updatebar : Form
    {
        ...
        private void lblcari_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                query = "SELECT * FROM tbl_item_bar WHERE id_item_bar = @id_item_bar";
                perintah = new MySqlCommand(query, koneksi);
                perintah.Parameters.AddWithValue("@id_item_bar", id);
                ...
                if (reader.Read())
                {
                    ...
                    txtid.Text = reader["id_item_bar"].ToString();
        ...
        private void lblupdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    query = "UPDATE tbl_item_bar SET nama_item = @nama_item, kategori = @kategori, stok = @stok WHERE id_item_bar = @id_item_bar";
                    perintah = new MySqlCommand(query, koneksi);
        ...
        private void updatebar_Load(object sender, EventArgs e)
        {
            try
            {
                koneksi.Open();
        ...
        private void lbldelet_Click(object sender, EventArgs e)
        {
            try
            {
                // Mendapatkan nilai id dari txtid (textbox yang berisi id_item_bar untuk dihapus)
                string id = txtid.Text;
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    query = "DELETE FROM tbl_item_bar WHERE id_item_bar = @id_item_bar";
                    perintah = new MySqlCommand(query, koneksi);
                    perintah.Parameters.AddWithValue("@id_item_bar", id);
